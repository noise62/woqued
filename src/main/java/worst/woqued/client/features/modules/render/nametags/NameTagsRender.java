...
public void onRender3D(Render3DEvent.Render3DEventData event) {
    ...
    private void renderName(Entity entity, float x, float y, DrawContext context) {
        ...
        RenderUtil.BLUR_RECT.draw(matrixStack, x, y, textWidth + gap * 2f, size + gap * 2f, scale, UIColors.widgetBlur(), 1f - module.glassy.getValue());
        ...
        font.drawGradientText(matrixStack, name, textX, textY, size, UIColors.primary(), UIColors.secondary(), nameWidth / 4f);

        if (hasHealth) {
            textX += nameWidth + gap;
            iconFont.drawGradientText(matrixStack, "d", textX, textY, iconSize, UIColors.primary(), UIColors.secondary(), healthIconWidth / 4f);
            textX += healthIconWidth + gap;
            font.drawGradientText(matrixStack, healthText, textX, textY, size, UIColors.primary(), UIColors.secondary(), healthTextWidth / 4f);
        }
    }
