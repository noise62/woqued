...
import worst.woqued.api.event.events.player.world.BlockPlaceEvent;
...
import worst.woqued.api.utils.rotation.RaytracingUtil;
...
public class AutoExplosionModule extends Module {
    ...
    private Rotation rotate(Entity entity) {
        ...
        RotationStrategy configurable = new RotationStrategy(getAimMode(), AuraModule.getInstance().moveCorrection.is("Focus"), AuraModule.getInstance().moveCorrection.is("Free")).ticksUntilReset(attackDelay.getValue().intValue());
